...
export async function step7(
  ...
  console.log(
    `\tAAVE_LENDING_POOL_CORE_ADDRESS used = ${consts.misc.AAVE_LENDING_POOL_CORE_ADDRESS}`
  );
  ...
  if (!["kovan", "mainnet"].includes(hre.network.name)) {
    ...
    console.log(
      "[NOTICE - TODO] We will need to use the governance multisig to setForgeFactoryValidity for Aave"
    );
