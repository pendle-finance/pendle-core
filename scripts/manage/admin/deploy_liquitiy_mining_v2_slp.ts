...
async function main() {
  ...
  const pid = BN.from(process.argv[9]);
  ...
  console.log(`About to deploy lmV2Slp`);
  const lmV2Slp = await deploy(hre, deployment, 'PendleSLPLiquidityMining', [
  ...
  await sendAndWaitForTransaction(hre, lmV2Slp.fund, 'fund epoch 1', [
