...
import {constants, tokens, amountToWei, getAContract, advanceTime, getLiquidityRate, getGain, resetChain, evm_revert, evm_snapshot} from "../helpers"
...
describe('Benchmark', async () => {
    ...
    it('[After 1 month] should be able to redeem aUSDT to get back OT, XYT and interests $', async () => {
      ...
      expect(finalAUSDTbalance.toNumber()).to.be.approximately(initialAUSDTbalance.add(gain).toNumber(), 10);
    });
    ...
    it('[After 1 month] should be able to get due interests', async () => {
       ...
       expect(finalAUSDTbalance.toNumber()).to.be.approximately(afterLendingAUSDTbalance.add(gain).toNumber(), 10);
      });
      ...
      it('After expiry, should be able to redeem aUSDT from OT', async () => {
        ...
        const duration = constants.TEST_EXPIRY.sub(Math.round(Date.now() / 1000)).sub(constants.ONE_DAY);
