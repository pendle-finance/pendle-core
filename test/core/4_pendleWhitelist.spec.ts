...
describe('pendleWhitelist', async () => {
  ...
  before(async () => {
    ...
    mockPendleWhitelistContract = await deployContract(alice, MockPendleWhitelist, [pendleGovernanceManagerContract.address]);
  ...
  it('should be able to add to whitelist, and emit corresponding events', async () => {
    ...
    .to.emit(pendleWhitelistContract, "AddedToWhiteList").withArgs(bob.address)
    .to.emit(pendleWhitelistContract, "AddedToWhiteList").withArgs(charlie.address);
    const whitelist = await pendleWhitelistContract.getWhitelist();
  ...
  it('should not be able to add zero address to whitelist', async () => {
    await expect(pendleWhitelistContract.connect(governance).addToWhitelist([bob.address, consts.ZERO_ADDRESS])).to.be.revertedWith("ZERO_ADDRESS");
  })
  ...
  it('should not be able to add duplicate address to whitelist', async () => {
    ...
    await expect(pendleWhitelistContract.connect(governance).addToWhitelist([charlie.address, dave.address])).to.be.revertedWith("ALREADY_WHITELISTED");
  })

  it('non-governance address shall not be able to add to or remove from whitelist', async() => {
    await expect(pendleWhitelistContract.connect(alice).addToWhitelist([alice.address])).to.be.revertedWith("ONLY_GOVERNANCE");
    await expect(pendleWhitelistContract.connect(alice).removeFromWhitelist([bob.address])).to.be.revertedWith("ONLY_GOVERNANCE");
  })
  ...
  it('should be able to remove from whitelist, and emit corresponding eevnts', async() => {
    ...
    await expect(pendleWhitelistContract.connect(governance).removeFromWhitelist([alice.address])).to.emit(pendleWhitelistContract, "RemovedFromWhiteList").withArgs(alice.address);
    ...
    await expect(pendleWhitelistContract.connect(governance).removeFromWhitelist([bob.address, charlie.address]))
      .to.emit(pendleWhitelistContract, "RemovedFromWhiteList").withArgs(bob.address)
      .to.emit(pendleWhitelistContract, "RemovedFromWhiteList").withArgs(charlie.address);
  ...
  it('should not be able to remove zero address from whitelist', async() => {
    await expect(pendleWhitelistContract.connect(governance).removeFromWhitelist([consts.ZERO_ADDRESS])).to.be.revertedWith("ZERO_ADDRESS");
  })

  it('should not be able to remove address that is not currently in whitelist', async() => {
    await pendleWhitelistContract.connect(governance).addToWhitelist([bob.address, charlie.address]);
    await expect(pendleWhitelistContract.connect(governance).removeFromWhitelist([bob.address, consts.DUMMY_ADDRESS])).to.be.revertedWith("NOT_WHITELISTED_YET");
  })

  it('should not be able to add or remove repeated address', async() => {
    await expect(pendleWhitelistContract.connect(governance).addToWhitelist([bob.address, bob.address])).to.be.revertedWith("ALREADY_WHITELISTED");
    await pendleWhitelistContract.connect(governance).addToWhitelist([bob.address, charlie.address]);
    await expect(pendleWhitelistContract.connect(governance).removeFromWhitelist([charlie.address, charlie.address])).to.be.revertedWith("NOT_WHITELISTED_YET");
  })
