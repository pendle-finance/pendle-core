...
describe('pendleGovernanceManager', async () => {
  ...
  it('should not be able to deploy zero address as governance', async () => {
    await expect(deployContract(alice, PendleGovernanceManager, [consts.ZERO_ADDRESS])).to.be.revertedWith("ZERO_ADDRESS")
  })

  it('should not be able to transfer governance to zero address', async () => {
    await expect(pendleGovernanceManagerContract.connect(alice).transferGovernance(consts.ZERO_ADDRESS)).to.be.revertedWith("ZERO_ADDRESS");
  })
