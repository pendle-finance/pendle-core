...
function shouldBehaveLikeERC20(errorPrefix, initialSupply, initialHolder, recipient, anotherAccount) {
  ...
  describe('transfer from', function () {
          ...
          describe('when the token owner does not have enough balance', function () {
            ...
            it('reverts', async function () {
              await expectRevert(
                ...
                errMsg.TRANSFER_EXCEED_BALANCE
              );
            });
          });
        });
    ...
    describe('when the token owner is the zero address', function () {
      ...
      it('reverts', async function () {
        await expectRevert(this.token.transferFrom(tokenOwner, to, amount, { from: spender }), errMsg.SENDER_ZERO_ADDR);
      });
    });
  });
...
function shouldBehaveLikeERC20Transfer(errorPrefix, from, to, balance, transfer) {
  describe('when the recipient is not the zero address', function () {
    describe('when the sender does not have enough balance', function () {
      ...
      it('reverts', async function () {
        await expectRevert(transfer.call(this, from, to, amount), errMsg.TRANSFER_EXCEED_BALANCE);
      });
    });
  ...
  describe('when the recipient is the zero address', function () {
    it('reverts', async function () {
      await expectRevert(transfer.call(this, from, ZERO_ADDRESS, balance), errMsg.RECEIVER_ZERO_ADDR);
    });
  });
}
...
function shouldBehaveLikeERC20Approve(errorPrefix, owner, spender, supply, approve) {
  ...
  describe('when the spender is the zero address', function () {
    it('reverts', async function () {
      await expectRevert(approve.call(this, owner, ZERO_ADDRESS, supply), errMsg.SPENDER_ZERO_ADDR);
    });
  });
}
