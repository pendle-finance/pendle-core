...
*/
abstract contract PendleLiquidityMiningBaseMulti is
    ...
    ReentrancyGuard
{
    ...
    constructor(ConstructorArgs memory args) PermissionsV2(args.governanceManager) {
        require(args.startTime > block.timestamp, "START_TIME_OVER");
        require(IERC20(args.underlyingAsset).totalSupply() > 0, "INVALID_ERC20");
        require(IERC20(args.baseToken).totalSupply() > 0, "INVALID_ERC20");
        require(args.vestingEpochs > 0, "INVALID_VESTING_EPOCHS");
        args.rewardTokens.verify();

        rewardTokens = args.rewardTokens;
        router = IPendleRouter(args.router);
        whitelist = IPendleWhitelist(args.whitelist);
        IPendleData dataTemp = IPendleRouter(args.router).data();
        data = dataTemp;
        require(
            dataTemp.getMarketFactoryAddress(args.marketFactoryId) != address(0),
            "INVALID_MARKET_FACTORY_ID"
        );
        require(dataTemp.getForgeAddress(args.forgeId) != address(0), "INVALID_FORGE_ID");

        address forgeTemp = dataTemp.getForgeAddress(args.forgeId);
        forge = forgeTemp;
        underlyingYieldToken = IPendleForge(forgeTemp).getYieldBearingToken(args.underlyingAsset);
        pausingManager = IPendlePausingManager(args.pausingManager);
        marketFactoryId = args.marketFactoryId;
     ...
     */
    function fund(PairUints[] calldata _rewards) external override onlyGovernance {
        checkNotPaused();
        ...
        require(!totalFunded.allZero(), "ZERO_FUND");
        funded = true;
        numberOfEpochs = numberOfEpochs.add(nNewEpochs);
        rewardTokens.safeTransferFrom(msg.sender, address(this), totalFunded);
    ...
    */
    function topUpRewards(uint256[] calldata _epochIds, PairUints[] calldata _rewards)
        external
        override
        onlyGovernance
        isFunded
    {
        ...
        require(!totalTopUp.allZero(), "ZERO_FUND");
        rewardTokens.safeTransferFrom(msg.sender, address(this), totalTopUp);
    ...
    /**
     * @notice Use to stake LPs to a specific expiry
    ...
    {
        checkNotPaused();
        newLpHoldingContractAddress = _stake(forAddr, msg.sender, expiry, amount);
    }
        ...
        returns (address newLpHoldingContractAddress)
    {
        ...
        newLpHoldingContractAddress = _stake(msg.sender, msg.sender, expiry, amount);
    }
    ...
    function withdrawTo(
        ...
        _pushLpToken(msg.sender, to, expiry, amount);
        ...
        returns (PairUints memory rewards)
    {
        ...
        rewardTokens.safeTransfer(user, rewards);
    }
    ...
    function readEpochData(uint256 epochId)
        external
        view
        returns (uint256 settingId, PairUints memory totalRewards)
    {
    ...
    function readAvailableRewardsForUser(uint256 epochId, address user)
        external
        view
        returns (PairUints memory availableRewardsForUser)
    {
    ...
    function _stake(
        ...
        if (!userExpiries[forAddr].hasExpiry[expiry]) {
            userExpiries[forAddr].expiries.push(expiry);
            userExpiries[forAddr].hasExpiry[expiry] = true;
        }

        _pullLpToken(forAddr, payer, marketAddress, expiry, amount);
        emit Staked(expiry, forAddr, amount);
    }
    ...
    */
    ...
    function _calcAmountRewardsForUserInEpoch(
        ...
        address user,
        uint256 epochId
    ) internal view returns (PairUints memory rewardsPerVestingEpoch) {
        uint256 settingId = epochId >= latestSetting.firstEpochToApply
            ? latestSetting.id
            : epochData[epochId].settingId;

        PairUints memory rewardsForThisExpiry = epochData[epochId]
            .totalRewards
    ...
    function _pullLpToken(
        ...
        _updatePendingRewards(expiry, forAddr);
        _updateDueInterests(expiry, forAddr);

        ExpiryData storage exd = expiryData[expiry];
        exd.balances[forAddr] = exd.balances[forAddr].add(amount);
        exd.totalStakeLP = exd.totalStakeLP.add(amount);

        IERC20(marketAddress).safeTransferFrom(payer, expiryData[expiry].lpHolder, amount);
    }
    ...
    function _pushLpToken(
        ...
        _updatePendingRewards(expiry, payer);
        _updateDueInterests(expiry, payer);

        ExpiryData storage exd = expiryData[expiry];
        exd.balances[payer] = exd.balances[payer].sub(amount);
        exd.totalStakeLP = exd.totalStakeLP.sub(amount);

        IPendleLpHolder(expiryData[expiry].lpHolder).sendLp(receiver, amount);
    }
    ...
    */
    ...
    function _allowedToWithdraw(address _token) internal view override returns (bool allowed) {
        allowed = (rewardTokens.contains(_token) == false);
    }
