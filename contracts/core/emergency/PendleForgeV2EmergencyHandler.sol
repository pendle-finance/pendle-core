...
/*
- This contract is very similar to PendleForgeEmergencyHandler
*/
contract PendleForgeV2EmergencyHandler is PermissionsV2, ReentrancyGuard {
    ...
    constructor(address _governanceManager, address _data) PermissionsV2(_governanceManager) {
    ...
    function setUpEmergencyMode(
        ...
        IPendleForgeV2 forge = IPendleForgeV2(_forgeAddr);
        forge.setUpEmergencyModeV2(_underlyingAsset, _expiry, address(this), extraFlag);
    ...
    */
    function withdraw(
        ...
        uint256 amountYieldTokenOut = otProportion.rmul(
            fod.yieldToken.balanceOf(address(fod.yieldTokenHolder))
        );
        fod.yieldToken.safeTransferFrom(
            address(fod.yieldTokenHolder),
            msg.sender,
            amountYieldTokenOut
        );

        uint256 amountRewardOut = otProportion.rmul(
            fod.rewardToken.balanceOf(address(fod.yieldTokenHolder))
        );
        fod.rewardToken.safeTransferFrom(
            address(fod.yieldTokenHolder),
            msg.sender,
            amountRewardOut
        );
